<?php
  ...
  if (!is_array($data)) {
      //delete data
      $statusdownload->kodepabrik = isset($data->kodepabrik) ? urldecode($data->kodepabrik) : null;
      $statusdownload->kodegudang = isset($data->kodegudang) ? urldecode($data->kodegudang) : null;
      $statusdownload->deletestatuspabrik();

      //insert data
      $statusdownload->kodepabrik = isset($data->kodepabrik) ? urldecode($data->kodepabrik) : null;
      $statusdownload->kodegudang = isset($data->kodegudang) ? urldecode($data->kodegudang) : null;
      if ($statusdownload->createstatuspabrik()) {
          $success++;
      } else {
          $fail++;
      }
    ...
    foreach ($data as $item) {
        //delete data
        $statusdownload->kodepabrik = isset($item->kodepabrik) ? urldecode($item->kodepabrik) : null;
