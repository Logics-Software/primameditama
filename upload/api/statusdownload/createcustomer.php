<?php
  ...
  if (!is_array($data)) {
      //delete data
      $statusdownload->kodecustomer = isset($data->kodecustomer) ? urldecode($data->kodecustomer) : null;
      $statusdownload->kodegudang = isset($data->kodegudang) ? urldecode($data->kodegudang) : null;
      $statusdownload->deletestatuscustomer();

      //insert data
      $statusdownload->kodecustomer = isset($data->kodecustomer) ? urldecode($data->kodecustomer) : null;
      $statusdownload->kodegudang = isset($data->kodegudang) ? urldecode($data->kodegudang) : null;
      if ($statusdownload->createstatuscustomer()) {
          $success++;
      } else {
          $fail++;
      }
    ...
    foreach ($data as $item) {
        //delete data
        $statusdownload->kodecustomer = isset($item->kodecustomer) ? urldecode($item->kodecustomer) : null;
